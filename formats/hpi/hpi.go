...
// Writer writes HPI archives (stub implementation)
type Writer struct {
	file *os.File
	root *Entry
}

// CreateWriter creates a new HPI archive
...
func (w *Writer) AddFile(archivePath, filePath string) error {
	return errors.New("writer not yet implemented")
}
...
// AddDirectory adds a directory recursively.
func (w *Writer) AddDirectory(archivePath, dirPath string) error {
	return errors.New("writer not yet implemented")
}

// write writes the HPI archive.
